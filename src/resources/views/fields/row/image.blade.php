    ...
    @if(!empty($value))
                ...
                <div class="flex -space-x-2">
                    @foreach(array_slice($value, 0, 4) as $image)
                        <span class="relative inline-flex items-center justify-center w-10 text-white rounded-full">
                            <img src="{{ asset($image) }}" alt="{{ $image }}" class="border-2 border-white rounded-full aspect-square object-cover hover:scale-[2]" />
                        </span>
                    @endforeach
                    @if(count($value) > 4)
        ...
        @else
            <span class="relative inline-flex items-center justify-center w-10 text-white rounded-full group">
                <img src="{{ asset($value) }}" alt="{{ $value }}" class="border-2 border-white rounded-full aspect-square object-cover hover:scale-[2]" />
            </span>
        @endif
    @endif
</div>
