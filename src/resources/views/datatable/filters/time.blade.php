<div class="border border-2 border-slate-200 flex flex-col rounded overflow-hidden mb-3 dark:border-slate-600">
    ...
    <div x-data="{model: @entangle('filters.' . $key).defer}"
         ...
         wire:ignore
         class="relative">
        <input type="time" wire:model.defer="filters.{{$key}}" id="{{ $uid }}" placeholder="..."
               class="relative border-0 group block w-full dark:bg-slate-400 dark:text-slate-100 dgDateFilter dgDateFilter{{ ucfirst(\Illuminate\Support\Str::camel($key)) }}"/>
    </div>
</div>
