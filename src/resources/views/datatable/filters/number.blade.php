<div x-data="{currentValue: @entangle('filters.'.$key).defer}"
     class="border border-2 border-slate-200 flex flex-col rounded overflow-hidden mb-3 dark:border-slate-600">
    ...
    <div class="relative group p-1 flex flex-row justify-center items-center dark:bg-slate-400 dark:text-slate-100">
        ...
        <input type="text" x-ref="numberField" x-model.number="currentValue" wire:model.defer="filters.{{$key}}" id="{{ $key }}" placeholder="0"
               class="relative border-0 group block w-full appearance-none ring-0 focus:ring-0 hover:ring-0 text-center dark:bg-slate-400" />
