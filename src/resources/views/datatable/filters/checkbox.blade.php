        ...
        @foreach($options as $optionKey => $optionValue)
            <div class="mt-1">
                <label class="inline-flex items-center">
                    <input type="checkbox" value="{{ $optionKey }}" wire:model.defer="filters.{{$key}}.{{$optionKey}}" name="{{$key}}"  class="focus:ring-0 h-5 w-5 text-indigo-600 border-gray-300 rounded dark:bg-slate-500 dark:border-slate-500">
